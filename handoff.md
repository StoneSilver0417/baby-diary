...
- **테스트 데이터 미정리**: 실 프로덕션 E2E 검증에 쓴 테스트 계정 3개(`bd-e2e-test-1/2/3-20260706@example.com`)와 "테스트가족" household가 아직 DB에 남아있음. 정리 SQL:
  ...
  ```
- **관리자 계정으로 직접 확인 필요**: `/admin`에서 문의 목록·답변 작성·`admin_stats` 수치가 정상인지는 AI가 관리자 비밀번호를 가질 수 없어 직접 확인 못함 — 설정 탭 "관리자 페이지" 링크 노출 여부와 함께 확인 필요.
...
1. [ ] 관리자 계정으로 `/admin` 문의 답변 작성 직접 확인
2. [ ] 안드로이드 실기기에 최신 APK 설치 후 다자녀 스위처·일기 대상 선택·기존 UX(뒤로가기·시트 여백·사진 스와이프) 전체 재검증
