...
- **Vercel 프로덕션 배포**: 사용자가 "아이폰은 어떻게 설치?"라고 물어본 데서 시작 — 아이폰은 PWA만 가능하고 PWA는 HTTPS로 배포된 URL이 있어야 홈화면 추가·서비스워커가 동작하므로 배포를 진행.
  - `npx vercel link --yes --project baby-diary`로 프로젝트 생성(GitHub 저장소 자동 연결은 실패했지만 CLI 직접 배포에는 지장 없음 — 필요 시 나중에 Vercel 대시보드에서 GitHub 연동 가능).
  ...
  - `npx vercel --prod --yes`로 배포 성공 → https://baby-diary-tau.vercel.app 발급. Playwright로 실제 프로덕션 URL에 접속해 로그인 화면 정상 렌더(콘솔 에러 0건), `manifest.webmanifest` 200 응답, 서비스워커 실제 등록(`navigator.serviceWorker.getRegistrations().length === 1`) 확인.
...
- **Vercel 프로덕션 URL이 GitHub 저장소와 연결돼 있지 않음** — 지금은 `vercel --prod`를 수동 실행해야 재배포됨(자동 배포 아님). GitHub 연동은 Vercel 대시보드에서 저장소 접근 권한을 다시 부여하면 해결 가능(원인 불명확 — 이번엔 CLI 직접 배포로 우회).
...
5. [ ] (정식 배포 시) Android release 서명 키스토어 생성 + release 빌드 서명 설정, Vercel-GitHub 자동 배포 연동.
