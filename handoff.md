...
- **버전**: v0.7.0 (Capacitor 8 Android 추가, 디버그 APK 빌드 성공)
- **빌드 상태**: `npx tsc --noEmit` 통과, `npm run build` 통과, `cd android && ./gradlew.bat assembleDebug` 통과(2분 31초, `android/app/build/outputs/apk/debug/app-debug.apk` 8.2MB 생성 확인)
...
- **안드로이드 실기기 검증 미완료**: 이 세션에 실물 안드로이드 기기가 없어 APK를 실제로 설치해 사진 선택·업로드 등 전 플로우를 확인하지 못함 — 다음 세션에서 실기기 확인 필요.
...
1. [ ] 안드로이드 실기기에 `android/app/build/outputs/apk/debug/app-debug.apk` 수동 설치 → 로그인·사진 업로드(시스템 Photo Picker 동작)·전체 플로우 검증
