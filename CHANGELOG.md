...
- **원인**: 안드로이드 Chrome/WebView는 좁은 텍스트 칼럼의 가독성을 위해 폰트 크기를 자동으로 확대하는 "text-size-adjust(폰트 부스팅)" 기능이 기본 켜져 있고, 이 확대 비율은 **기기 화면 크기·밀도별 휴리스틱**으로 계산돼 기기마다 다르다. `paper-lines` 배경의 줄 간격은 CSS에 지정된 고정 `rem` 값으로 그려지는데, 실제 텍스트 줄 높이는 이 자동 부스팅으로 조금씩 더 커지므로 몇 줄만 지나도 배경 줄과 글자가 어긋난다 — 정확히 "기기마다 다르게, 줄이 이어질수록 더 어긋나는" 증상과 일치.
