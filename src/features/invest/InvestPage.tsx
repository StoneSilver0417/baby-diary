import { useState } from 'react'
import { format } from 'date-fns'
import { Plus } from 'lucide-react'
import {
  Sheet,
  SheetContent,
  SheetHeader,
  SheetTitle,
  SheetTrigger,
} from '@/components/ui/sheet'
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs'
import { useChild, useProfiles } from '@/features/diary/useDiaryQueries'
import { computeHoldings } from './holdings'
import { useNotes, useTrades } from './useInvestQueries'
import { TradeForm } from './TradeForm'
import { NoteForm } from './NoteForm'

type TimelineItem =
  | { type: 'trade'; date: string; id: string; label: string }
  | { type: 'note'; date: string; id: string; label: string; authorName: string }

export function InvestPage() {
  const { data: child } = useChild()
  const { data: profiles } = useProfiles()
  const { data: trades } = useTrades()
  const { data: notes } = useNotes()
  const [sheetOpen, setSheetOpen] = useState(false)

  const holdings = computeHoldings(trades ?? [])

  const timeline: TimelineItem[] = [
    ...(trades ?? []).map((t) => ({
      type: 'trade' as const,
      date: t.trade_date,
      id: t.id,
      label: `${t.side} · ${t.stock_name} ${t.quantity}주 @${t.unit_price.toLocaleString()}${t.memo ? ` · ${t.memo}` : ''}`,
    })),
    ...(notes ?? []).map((n) => ({
      type: 'note' as const,
      date: n.note_date,
      id: n.id,
      label: n.content,
      authorName: profiles?.find((p) => p.id === n.author_id)?.display_name ?? '',
    })),
  ].sort((a, b) => b.date.localeCompare(a.date))

  return (
    <div className="min-h-full pt-safe">
      <header className="p-5">
        <h1 className="text-xl font-semibold text-foreground">{child?.name} 투자일기</h1>
      </header>

      <section className="mx-5 mb-5 space-y-3 rounded-lg border border-border bg-card p-4">
        {holdings.length === 0 && (
          <p className="text-sm text-muted-foreground">아직 보유 중인 종목이 없어요.</p>
        )}
        {holdings.map((h) => (
          <div key={h.stockName} className="flex items-center justify-between text-sm">
            <span className="font-medium text-foreground">{h.stockName}</span>
            <div className="text-right text-muted-foreground">
              <div>
                {h.quantity}주 · 평단 {h.avgUnitPrice.toLocaleString()}원
              </div>
              <div>투입원금 {h.principal.toLocaleString()}원</div>
            </div>
          </div>
        ))}
      </section>

      <div className="divide-y divide-border">
        {timeline.map((item) => (
          <div key={`${item.type}-${item.id}`} className="p-5">
            <div className="mb-1 flex items-center gap-2 text-xs text-muted-foreground">
              <span>{format(new Date(item.date), 'M월 d일')}</span>
              {item.type === 'note' && <span>{item.authorName}</span>}
            </div>
            <p className="text-sm text-foreground">{item.label}</p>
          </div>
        ))}
        {timeline.length === 0 && (
          <p className="p-8 text-center text-sm text-muted-foreground">
            아직 기록된 거래·메모가 없어요.
          </p>
        )}
      </div>

      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetTrigger asChild>
          <button
            type="button"
            className="fixed bottom-24 right-5 flex size-14 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-lg"
            aria-label="거래·메모 추가"
          >
            <Plus className="size-6" />
          </button>
        </SheetTrigger>
        <SheetContent side="bottom" className="max-h-[85vh] overflow-y-auto">
          <SheetHeader>
            <SheetTitle>거래·메모 추가</SheetTitle>
          </SheetHeader>
          <div className="p-4 pt-0">
            <Tabs defaultValue="trade">
              <TabsList className="w-full">
                <TabsTrigger value="trade" className="flex-1">
                  거래 기록
                </TabsTrigger>
                <TabsTrigger value="note" className="flex-1">
                  메모
                </TabsTrigger>
              </TabsList>
              <TabsContent value="trade" className="pt-4">
                {child && <TradeForm childId={child.id} onDone={() => setSheetOpen(false)} />}
              </TabsContent>
              <TabsContent value="note" className="pt-4">
                <NoteForm onDone={() => setSheetOpen(false)} />
              </TabsContent>
            </Tabs>
          </div>
        </SheetContent>
      </Sheet>
    </div>
  )
}
