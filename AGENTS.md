...
- 배포: 안드로이드 = PWA(웹앱 설치, `vite-plugin-pwa`) / 아이폰 = PWA(홈화면 추가) / 웹 = Vercel. **v0.11.0부터 Capacitor/APK 배포 완전 철거** — 안드로이드도 아이폰과 동일하게 PWA 단일 경로(이전엔 Capacitor 디버그 APK를 GitHub Release로 병행 배포했으나, 설치자 자동 업데이트 불가·수동 3단계 배포 부담으로 제거).
- **저장소는 Public**(v0.10.3부터, 지인 배포 시 GitHub 로그인 없이 접근 가능하게 하려고 전환) — 커밋 전 항상 시크릿 유무 재확인할 것(`.env.local`은 gitignore 대상, 지금까지 커밋 이력 없음 확인됨).

## 주요 명령어
```bash
npm run dev      # 개발 서버
npm run build    # 빌드 (PWA manifest·서비스워커 포함) — push하면 Vercel이 자동 배포
npx tsc --noEmit # 타입 체크
npm run icons    # src/assets/app-icon.png → public/icons/* 재생성 (sharp)
```
...
- 프로젝트 경로에 한글 폴더 사용 금지 — 이 폴더가 `baby-diary`인 이유(과거 Android gradle 빌드의 Windows 인코딩 이슈로 정한 규칙, v0.11.0에서 Capacitor 철거 후에도 관성 유지).
